...
//
    ...
    func encodeJSON(from user: User) -> String{
        do{
            let encoder = JSONEncoder()
            encoder.dateEncodingStrategy = .iso8601
    ...
    func editUser(from user: User) async {
        ...
        guard let url = URL(string: "https://hopeful-creativity-production.up.railway.app/api/user/editUser") else {
            print("URL inválida")
            return
        }
        ...
        do {
            let encoder = JSONEncoder()
            encoder.dateEncodingStrategy = .iso8601
            let jsonData = try encoder.encode(userToUpdate)
            request.httpBody = jsonData
